...
fwp_run(const char *id, const char *ifsrc, const char *ifdst)
{
    ...
    while (!fwp_quit) {
        ...
        if (fds[src].revents & POLLIN) { // TODO
            ...
            if (desc->hdr.bh1.block_status & TP_STATUS_USER) {
             ...
             //     int size = ppd->tp_snaplen; // TODO
                    ...
                    if (!memcmp(&pkt->eth.proto, &proto_vlan, 2)) {
        ...
        if (fds[2].revents & POLLIN) {
            ...
            if (msg.vlan <= 0xFFF) {
                fprintf(stderr, "New rule: vlan %u ip %d.%d.%d.%d mask %d.%d.%d.%d\n",
                        msg.vlan,
                        (msg.reply.ip        ) & 255,
                        (msg.reply.ip   >>  8) & 255,
                        (msg.reply.ip   >> 16) & 255,
...
static int
fwp_set(const char *id, const char *ip, const char *vlan)
{
    struct fwp_msg msg = {
        .op = 1,
        .vlan = -1,
...
main(int argc, char **argv)
{
    ...
    if (argc >= 2 && !strcmp(argv[1], "set")) {
        if (argc != 4 && argc != 5) {
            printf("usage: %s set ID CIDR [VLAN]\n", argv[0]);
            return 1;
        }
