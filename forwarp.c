        ...
        if ((fds[dst].revents & POLLIN)) {
            ...
            if (desc->hdr.bh1.block_status & TP_STATUS_USER) {
                ...
                for (unsigned i = 0; i < num_pkts; i++) {
                    ...
                    fprintf(stderr, "it's a reply, setting arp source %02x:%02x:%02x:%02x:%02x:%02x as reachable on vlan %d\n",
...
static int
fwp_set(const char *id, const char *ip, const char *vlan, const char *name)
{
    struct fwp_msg msg = {
        .op = 1,
        .vlan = -1,
    ...
    if (vlan) {
        if (uparse(vlan, &msg.vlan, 4095)[0] || msg.vlan == -1) {
            fprintf(stderr, "Unable to parse VLAN %s\n", vlan);
            return 1;
        }
        msg.reply.index = if_nametoindex(name); // TODO
...
main(int argc, char **argv)
{
    ...
    if (argc >= 2 && !strcmp(argv[1], "set")) {
        if (argc == 4) {
            return fwp_set(argv[2], argv[3], NULL, NULL);
        } else if (argc == 6) {
            return fwp_set(argv[2], argv[3], argv[4], argv[5]);
        } else {
            printf("usage: %s set ID CIDR [VLAN IFACE]\n", argv[0]);
            return 1;
        }
    }
